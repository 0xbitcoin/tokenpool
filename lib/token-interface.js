...
module.exports =  {
  ...
  async init(redisInterface, web3, accountConfig, poolConfig, pool_env )
  {
    ...
    if (cluster.isMaster) {
      ...
      // initialize to zero to make sure we read the most recent block in collectTokenParameters
      this.redisInterface.storeRedisData('ethBlockNumber', 0 );
    }
  ...
  async collectTokenParameters( )
  {
    ...
    if (challengeNumber != this.challengeNumber) {
      ...
      if (!seenBefore) {
        ...
        this.ipcServer.broadcast('challengeNumber', challengeNumber);
      } else {
      ...
      var web3 = this.web3;
      var lastBlockNumber = parseInt( await this.redisInterface.loadRedisData('ethBlockNumber' ));
      ...
      // sometimes an Infura node might be lagging the rest of the network and give us an old block #, so
      // make sure it is greater than what we currently have.
      if (ethBlockNumber > lastBlockNumber) {
        this.redisInterface.storeRedisData('ethBlockNumber', ethBlockNumber )
        this.ipcServer.broadcast('ethBlockNumber', ethBlockNumber);
      }
  },
