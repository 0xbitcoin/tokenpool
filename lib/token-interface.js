...
module.exports =  {
  ...
  async sendMiningSolutions()
    ...
    //  console.log( 'sendMiningSolutions' )
      if(busySendingSolution == false)
      {
        if(queuedMiningSolutions.length > 0)
        {
          ...
          var hasSubmittedChallenge = await this.hasSubmittedSolutionWithChallengeNumber()
    ...
    async hasSubmittedSolutionWithChallengeNumber(challengeNumber)
    {
      ...
      for(i in solution_txes)
      {
        ...
        if( transactionData.challengeNumber === challengeNumber  )
        {
            console.log('has submitted soln !! ')
          return true
        }
