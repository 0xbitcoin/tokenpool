 ...
 async  update()
  {
   ...
   transactionCoordinator.init(this.web3,this.tokenContract,this.poolConfig,this.accountConfig,this.redisInterface,this)
    ...
    await self.collectTokenParameters();
     ...
     setTimeout(function(){ self.transferMinimumTokensToPayoutWallet()} , 15*60*1000 ) // 15 min
  ...
  async collectTokenParameters( )
  {
    ...
    if (challengeNumber != this.challengeNumber) {
      ...
      /*
      tokenPricePerEth:  await this.redisInterface.loadRedisData('tokenPricePerEth' )
      lastMintedBlock:  await this.redisInterface.loadRedisData('lastMintedBlock' )
      totalPoolHashrate:  await this.redisInterface.loadRedisData('totalPoolHashrate' )
      activeMinersCount:  await this.redisInterface.loadRedisData('activeMinersCount' )
      blocksPerHour:  await this.redisInterface.loadRedisData('blocksPerHour' )
...
//minerEthAddress
  ...
  /*
    ...
    */
    async transferMinimumTokensToPayoutWallet()
    {
      ...
      var self = this;
      setTimeout(function(){ self.transferMinimumTokensToPayoutWallet()}, 15*60*1000) //every 15 minutes
  ...
  var minerList =  await this.getMinerList()
    ...
    for(i in minerList) //reward each miner
    {
      var minerAddress = minerList[i];
       ...
       if(miner_token_balance > min_balance_for_transfer && minerAddress != null)
       {
  ...
  async saveMinerDataToRedis(minerEthAddress, minerData)
  {
    ...
    this.redisInterface.storeRedisHashData("miner_data_downcase", minerEthAddress.toString().toLowerCase() , JSON.stringify(minerData))
  ...
  async getMinerData(minerEthAddress)
  {
    ...
    var minerDataJSON = await this.redisInterface.findHashInRedis("miner_data_downcase", minerEthAddress.toString().toLowerCase() );
