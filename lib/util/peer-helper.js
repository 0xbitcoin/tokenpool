...
export default class PeerHelper {
  ...
  */
  ...
  static async getPoolFeesMetrics(poolConfig, mongoInterface){
    ...
    let tokenEthPriceRatio = await TokenDataHelper.getMineableTokenToEthPriceRatio(mongoInterface)

    if(!tokenEthPriceRatio || tokenEthPriceRatio == 0){
    ...
    let miningRewardInEth = (miningRewardFormatted * PeerHelper.constrainToPercent(tokenEthPriceRatio))
    ...
    return  {
      ...
      token_Eth_Price_Ratio: tokenEthPriceRatio, 
