...
static async saveSharesData(minerEthAddress, sharesData, mongoInterface)
  {
    ...
    await this.mongoInterface.upsertOne("miner_shares",{minerEthAddress: minerEthAddress},sharesData)
  ...
  async getSharesData(minerEthAddress)
  {
    if(minerEthAddress)
    {
      ...
      var sharesData = await this.mongoInterface.findOne("miner_shares", {minerEthAddress: minerEthAddress} );
     ...
     var total_shares = await PeerHelper.getTotalMinerShares(mongoInterface);
      ...
      for(var i in minerList) //reward each miner
      {
        var minerAddress = minerList[i];
         ...
         var minerData = await PeerHelper.getMinerData(minerAddress)
         var sharesData = await PeerHelper.getSharesData(minerAddress)
