...
export default class PeerHelper {
  ...
  /*  static async getAllTransactionData(mongoInterface)
   ...
   static   getPoolData(poolConfig)
    ...
    { 
      var payments = await mongoInterface.findAllSortedWithLimit('balance_payment',{minerEthAddress: minerAddress.toString().toLowerCase()}, {block:-1} , 250)
...
static async saveSharesData(minerEthAddress, sharesData, mongoInterface)
  ...
  static async awardTokensBalanceForShares( minerEthAddress, difficulty , mongoInterface)
  {
   ...
   //var sharesData = await PeerHelper.getSharesData(minerEthAddress,mongoInterface)
    ...
    let tokenRewardAmt = await PeerHelper.getTokenRewardForShareOfDifficulty(difficulty,mongoInterface)
    ...
    await mongoInterface.updateOneCustom('minerData', {_id: minerData._id}, {$inc:{alltimeTokenBalance: tokensAwarded}}   )
  ...
  */
  static async getTokenRewardForShareOfDifficulty(shareDiff, mongoInterface){
    ...
    let poolFeesFactor = await PeerHelper.getPoolFeesFactor(mongoInterface)

    let netBlockReward = totalBlockReward * poolFeesFactor
 ...
 //depends on amount of 0xBTC pool is paying in eth gas fees  
  static async getPoolFeesFactor(mongoInterface){
    return 0.5 
  }
