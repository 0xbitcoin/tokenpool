...
export default class Web3ApiHelper  {
  ...
  static async fetchAPIData(poolConfig,mongoInterface){
    ...
    try{  
      ...
      console.log("Fetch API Data from ethgasstation")
  ...
  static getGasPriceBoost( typename, poolConfig ){
    let configForType = (typename == 'solution' ) ? poolConfig.mintingConfig :  poolConfig.paymentsConfig

    let gasPriceBoost =  parseInt( configForType.gasPriceBoost ) 
    if(!isNaN(gasPriceBoost) && gasPriceBoost > 0){
      return  gasPriceBoost
    }
    return 0 
  }
  ...
  */
  static async getGasPriceWeiForTxType(typename, poolConfig, mongoInterface)
  {
    ...
    let gasPriceBoost = Web3ApiHelper.getGasPriceBoost(typename, poolConfig)
...
/*
    ...
    let maxGasPrice =  parseInt( configForType.maxGasPriceGwei ) 
