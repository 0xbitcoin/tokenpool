...
/*  async submitMiningSolutionWithHelper(minerAddress,solution_number,challenge_digest,challenge_number){
  ...
  */
  static async checkBatchPaymentsStatus( paymentContract, mongoInterface )
  {
...
/*
  ...
  static async storeEthereumTransaction(tx_hash,packetData)
  {

    console.log('storing data about eth tx ', tx_hash, packetData )

    
    let existingTransaction = await this.mongoInterface.findOne('transaction', {tx_hash: tx_hash})

    if(!existingTransaction){
      await this.mongoInterface.insertOne('transaction',   packetData )
    }else{
      await this.mongoInterface.updateOne('transaction', {tx_hash: tx_hash}, packetData )
    }
