...
/*
  ...
  try{
   ...
   var addressTo = this.mintHelperContract.options.address;
    ...
    let averageGasPriceWei = await this.getAverageGasPriceWei( )
    let maxSolutionGasPriceWei =  poolConfig.maxSolutionGasPriceWei

    let solutionGasPriceWei= maxSolutionGasPriceWei
    if(!isNaN(averageGasPriceWei)){
        solutionGasPriceWei = Math.min( averageGasPriceWei , maxSolutionGasPriceWei )
    }
  ...
  async transferPaymentBatch(batchPayment)
  {
   ...
   var txData = this.web3.eth.abi.encodeFunctionCall({
      ...
      /* }catch(e){
       ...
       let averageGasPriceWei = await this.getAverageGasPriceWei( )
       let maxTransferGasPriceWei =  poolConfig.maxTransferGasPriceWei

       let transferGasPriceWei= maxTransferGasPriceWei
       if(!isNaN(averageGasPriceWei)){
           transferGasPriceWei = Math.min( averageGasPriceWei , maxTransferGasPriceWei )
       }
       ...
       const txOptions = {
         nonce: web3utils.toHex(txCount),
         gas: web3utils.toHex( estimatedGasCost ),
         gasPrice: web3utils.toHex(web3utils.toWei(transferGasPriceWei.toString(), 'gwei') ),
         value: 0,
         to: addressTo,
         from: addressFrom,
         data: txData
