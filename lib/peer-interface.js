...
/*
  ...
  async update()
  {
    ...
    setTimeout(function(){this.processQueuedShares()}.bind(this),0)
     ...
     setTimeout(function(){this.monitorMinedSolutions()}.bind(this),0)


    //Error


     setTimeout(function(){this.calculateMinerHashrateData()}.bind(this),0)
...
/*
   ...
   */
   async calculateMinerHashrateData()
   {
        ...
        //  setTimeout(function(){self.calculateMinerHashrateData()},4000  )///perform after booting
         setTimeout(function(){this.calculateMinerHashrateData()}.bind(this),UPDATE_VAR_DIFF_PERIOD  )  // 1 minute
   } 
...
*/
  ...
  /* async loadMinerDataFromMongo(minerEthAddress)
   ...
   {
     var allMinerData = await this.getAllMinerData(mongoInterface);
  ...
  static async getAllMinerData(mongoInterface)
   {
     ...
     var minerList =  await this.getMinerList(mongoInterface)
     ...
     {
       var minerAddress = minerList[i];
       var minerData = await this.getMinerData(minerAddress, mongoInterface)
       var sharesData = await this.getSharesData(minerAddress, mongoInterface)
       ...
       console.log('listening on JSONRPC server localhost:8080')
         ...
         var server = jayson.server({
           ...
           getMinimumShareDifficulty: async function(args, callback) {
            ...
            var difficulty = await self.getPoolMinimumShareDifficulty( );
          ...
          //  var varDiff = await self.getMinerVarDiff(minerEthAddress);
            var varDiff = await self.getPoolMinimumShareDifficulty( );
