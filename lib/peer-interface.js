 ...
 //var redis = require("redis");
  ...
  const UPDATE_VAR_DIFF_PERIOD = 2 * 60  * 1000;
...
module.exports =  {
   ...
   */
   ...
   async  handleValidShare( nonce,minerEthAddress,digest,difficulty, shareIsASolution )
   {
      ...
      if(existingShare == null)
      {
        ...
        var shareData=  {block: ethBlock,nonce: nonce, miner: minerEthAddress, difficulty: difficulty, isSolution: shareIsASolution};
   ...
   estimateMinerHashrate(minerData, period)
   {
     ...
     if(isNaN(period)){period = UPDATE_VAR_DIFF_PERIOD}
     ...
     if(minerValidSubmittedSolutions >= 1){
       ...
       try {
          var timeToSolveABlock =  web3utils.toBN( Math.floor ( period / parseFloat(minerValidSubmittedSolutions) )) //seconds
   ...
   async processQueuedShares()
   {
     ...
     var shareData = await this.redisInterface.popFromRedisList("queued_shares_list")

     if(typeof shareData != 'undefined' && shareData != null)
     {  
        var response =  await self.handlePeerShareSubmit(shareData.nonce,shareData.minerEthAddress,shareData.hallengeNumber,shareData.digest,shareData.difficulty );
       }
  ...
  async  getShareCreditsFromDifficulty(difficulty,shareIsASolution)
   ...
   async getTotalMinerHashrate()
   {
     ...
     console.log('got miner total hashRate', hashrateSum)
     return hashrateSum;
  ...
  async initJSONRPCServer()
     {
       ...
       console.log('listening on JSONRPC server localhost:8586')
         ...
         getChallengeNumber: async function(args, callback) {
           ...
           var challenge_number = await this.redisInterface.loadRedisData('challengeNumber' )
