...
/*
  ...
  async update()
  {
    ...
    //Error
     ...
     setTimeout(function(){PeerInterface.calculateMinerHashrateData()}.bind(this),0)
  ...
  async processQueuedShares()
  {
    ...
    var shareData  = await this.mongoInterface.findAndDeleteOne('queued_shares_list')  //this.redisInterface.popFromRedisList("queued_shares_list")
    ...
    {
      try{
        var response =  await PeerInterface.handlePeerShareSubmit(shareData.nonce,shareData.minerEthAddress,shareData.challengeNumber,shareData.digest,shareData.difficulty,shareData.customVardiff, this.mongoInterface );
        }catch(err)
        {
  ...
  async monitorMinedSolutions()
  {
   ...
   console.log('error',e)
     ...
     setTimeout(function(){this.monitorMinedSolutions()}.bind(this),4000)
