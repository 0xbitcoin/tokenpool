...
//}
  ...
  async  getShareCreditsFromDifficulty(difficulty,shareIsASolution)
   ...
   {
     await this.redisInterface.storeRedisHashData("miner_data_downcase", minerEthAddress.toString().toLowerCase() , JSON.stringify(minerData))
   ...
   async getTotalMinerShares()
   {
     ...
     for(i in allMinerData)
     {
       ...
       var minerAddress = data.minerAddress.toString().toLowerCase();
   ...
   async getAllMinerData()
   {
     ...
     for(i in minerList)
     {
       var minerAddress = minerList[i].toString().toLowerCase();
       var minerData = await this.getMinerData(minerAddress)
