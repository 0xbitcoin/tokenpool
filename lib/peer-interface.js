 ...
 //var redis = require("redis");
   ...
   var tokenScout = require('./token-scout')
   ...
   async estimateMinerHashrate(minerAddress)
   {
     console.log('estimateMinerHashrate')
      try {
        ...
        console.log('meep', recentShareCount )
...
*/
  ...
  async  getShareCreditsFromDifficulty(difficulty,shareIsASolution)
   ...
   //build me
   async getTokenData(minerEthAddress)
   {
     if(minerEthAddress)
     {
       //var tokenDataJSON = await this.redisInterface.findHashInRedis("token_data", minerEthAddress.toString().toLowerCase() );

       var firstPunkData = await tokenScout.getFirstPunkDataForAddress( minerEthAddress, this.redisInterface );

      if(firstPunkData)
      {
        var firstPunkId = firstPunkData.punkIndex

        var tokenDataJSON = {
          punkId: firstPunkId,
          punkImageURL: tokenScout.getImageUrlForPunkId(firstPunkId)
        }
   ...
   async getAllMinerData()
   {
     ...
     for(i in minerList)
     {
       var minerAddress = minerList[i];
       var minerData = await this.getMinerData(minerAddress)
        var tokenData = await this.getTokenData(minerAddress)
