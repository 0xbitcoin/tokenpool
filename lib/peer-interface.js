...
module.exports =  {
   ...
   */
   ...
   async updateVariableDifficultyPeriod()
   {
     ...
     var self = this ;
           ...
           for(i in minerList) //reward each miner
           {
             ...
             minerData.hashRate = await this.estimateMinerHashrate(minerData,UPDATE_VAR_DIFF_PERIOD )
   ...
   async estimateMinerHashrate(minerData, period_ms)
   {
     var minerAddress = minerData.address;
     ...
     var sharesCount = submitted_shares.length;

     if(submitted_shares == null || sharesCount < 1)
     {
       return 0;
     }
     ...
     for (var i=0;i<sharesCount;i++)
     {
       ...
       summedHashrate += share.hashRateEstimate;
     }
