 ...
 //var redis = require("redis");
  ...
  const UPDATE_VAR_DIFF_PERIOD = 60 * 1000; 
      ...
      console.log("Update Vardiff")
          ...
          //  console.log( 'updateVariableDifficultyPeriod', minerList )
           ...
           for(i in minerList) //reward each miner
           {
             ...
             if (newVarDiff != minerData.varDiff) {
             	 // notify the stratum server
               this.ipcServer.broadcast('varDiff', JSON.stringify({
                    'miner'   : minerAddress, 
                    'varDiff' : newVarDiff,
                    'target'  : self.getTargetFromDifficulty(newVarDiff).toString()
               }));
             }
...
//}
  ...
  async  getShareCreditsFromDifficulty(difficulty,shareIsASolution)
   ...
   async loadMinerDataFromRedis(minerEthAddress)
   {
     ...
     {
       existingMinerData = peerUtils.getDefaultMinerData();
     }else{
