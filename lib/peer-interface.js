...
module.exports =  {
  ...
  async getAverageSolutionTime(minerAddress)
  {
   ...
   async checkMinedPayments(payment_txes)
   {
     ...
     var current_block = await this.redisInterface.getEthBlockNumber();

     for(i in payment_txes)
     {
       var tx_hash = payment_txes[i];


      var txDataJSON = await this.redisInterface.findHashInRedis('unconfirmed_broadcasted_payment',tx_hash);
      var transactionData = JSON.parse(txDataJSON)

      var paymentId = transactionData.balancePaymentId;

      if( transactionData.confirmed == false && (transactionData.last_broadcast_block < (currentEthBlock - 1000)) )
      {
