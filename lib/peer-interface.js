...
module.exports =  {
  ...
  async getAverageSolutionTime(minerAddress)
  {
   ...
   async cleanRedisData()
   {
     ...
     var lastSubmittedShareJSON = await this.redisInterface.peekLastFromRedisList('submitted_share'  );


     while(lastSubmittedShareJSON != null)
     {
        lastSubmittedShareJSON = await this.redisInterface.peekLastFromRedisList('submitted_share'  );
       var lastSubmittedShareData = JSON.parse(lastSubmittedShareJSON)

       if( lastSubmittedShareData.block < (currentEthBlock-DIGESTS_LIFETIME_BLOCKS) )
       {
         var shareData = await this.redisInterface.popLastFromRedisList('submitted_share'  );
         console.log('popping old share', shareData);
       }else{
         break;
       }
     }
