...
module.exports =  {
  ...
  /*   redisClient.set("string key", "string val", redis.print);
   ...
   getTargetFromDifficulty(difficulty)
   {
     ...
     var max_target = web3utils.toBN( 2 ).pow( web3utils.toBN( 234 ) ) ; //for real
  //   var max_target = web3utils.toBN( 2 ).pow( web3utils.toBN( 242 ) ) ; //for ropsten
   ...
   */
        ...
        redisClient.hkeys(key, function (err, replies) {
           if(err){rejected(err);return;}
            ...
            replies.forEach(function (reply, i) {
             ...
             console.log('found from redis ', results.toString())
            fulfilled(results);
            //redisClient.quit();
        });
   ...
   getMinerData(minerEthAddress)
   {
     if( minerEthAddress != null ){
       var minerDataJSON = await this.findHashInRedis("miner_data", minerEthAddress );
     }else {
       var minerDataJSON = await this.getResultsOfKeyInRedis("miner_data" );
     }


     return minerDataJSON;
   }
         ...
         getChallengeNumber: function(args, callback) {
          ...
          callback(null, self.tokenInterface.getPoolChallengeNumber().toString() );
           ...
           submitShare: function(args, callback) {
            ...
            callback(null, JSON.stringify( self.handlePeerShareSubmit(nonce,minerEthAddress,digest,difficulty ) ));

           },
           getMinerData: function(args, callback) {

             var minerEthAddress = args[0];

            callback(null, JSON.stringify( self.getMinerData(minerEthAddress) )  );
