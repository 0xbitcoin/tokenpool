  ...
  async getAverageSolutionTime(minerAddress)
  {
   ...
   async checkMinedPayments(payment_txes)
   {
    ...
    //  var paymentId = transactionData.balancePaymentId;
      ...
      if( (transactionData.last_broadcast_block < (currentEthBlock - REBROADCAST_WAIT_BLOCKS)) )
      {
        ...
        if(balanceTransferJSON == null || balanceTransfer.txHash == null)
        {
          ...
          await this.queueBalancePayment(transactionData)



            //why doesnt this occur !? 
        }else if(balanceTransfer.confirmed != true)
        {
   ...
   async queueBalancePayment(paymentData)
   {
     ...
     var REBROADCAST_WAIT_BLOCKS = 1000;

     //make sure only one replacement tx is being queued
     if( existingReplacementPayment == null /*|| (paymentData.last_broadcast_block < (currentEthBlock - REBROADCAST_WAIT_BLOCKS))*/ )
     {
