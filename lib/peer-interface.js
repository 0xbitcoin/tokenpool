...
/*
  ...
  async update()
  {
    ...
    setInterval(function(){this.processQueuedShares(this.mongoInterface, this.poolConfig)}.bind(this),4000)
  ...
  async processQueuedShares(mongoInterface, poolConfig)
  {
    ...
    {
      try{
        var response =  await PeerInterface.handlePeerShareSubmit(shareData.nonce,shareData.minerEthAddress,shareData.challengeNumber,shareData.digest,shareData.difficulty,shareData.customVardiff, mongoInterface , poolConfig);
        }catch(err)
        {
...
/*
   ...
   */
   ...
   //FIXME 
   static async handlePeerShareSubmit(nonce,minerEthAddress,challengeNumber,digest,difficulty,mongoInterface, poolConfig)
   {
     ...
     */
     ...
     var poolEthAddress = PeerHelper.getMintHelperAddress(poolConfig);
    ...
    if(computed_digest === digest &&
       ...
       //await this.redisInterface.storeRedisHashData("invalid_share", digest , JSON.stringify(shareData))
        ...
        await this.mongoInterface.insertOne("miner_shares", shareData )
  ...
  static async  handleValidShare( nonce,minerEthAddress,digest,difficulty, shareIsASolution, mongoInterface, poolConfig )
   {
      ...
      if(existingShare == null && minerEthAddress!=null)
      {
        ...
        var minerData = await PeerHelper.getMinerData(minerEthAddress.toString().toLowerCase())
       ...
       /* await this.redisInterface.pushToRedisList("submitted_shares_list", JSON.stringify(shareData))
        ...
        var challengeNumber = await PeerHelper.getPoolChallengeNumber( mongoInterface );
