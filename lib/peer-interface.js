...
module.exports =  {
  ...
  //  setInterval( function(){ self.monitorMinedSolutions()  } , 2*1000 )
   ...
   getMinerVarDiff(minerEthAddress)
   {
     var minDiff =  getPoolMinimumShareDifficulty()
   ...
   */
   ...
   async updateVariableDifficultyPeriod()
   {
           ...
           for(i in minerList) //reward each miner
           {
             ...
             var newVarDiff = getUpdatedVarDiffForMiner(minerData)
          ...
          varDiffPeriodCount++;
         setTimeout(function(){self.updateVariableDifficultyPeriod()},5 * 60 * 1000)  // 1 minute
   }

   //we expect a solution per minute ??
   async getUpdatedVarDiffForMiner(minerData)
   {
   ...
   async monitorMinedSolutions()
   {
     ...
     if( solution_txes != null && solution_txes.length > 0)
     {
        await this.checkMinedSolutions( solution_txes )
         ...
         setTimeout(function(){self.monitorMinedSolutions()},2000)
       ...
       console.log('listening on JSONRPC server localhost:8586')
         ...
         var server = jayson.server({
           ...
           getMinimumShareDifficulty: function(args, callback) {
            ...
            callback(null, self.getMinerVarDiff(minerEthAddress).toString() );
