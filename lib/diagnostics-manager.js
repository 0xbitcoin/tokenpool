...
//pool will not suspend under this gas price 
const POOL_SUSPENSION_MINIMUM_GAS_PRICE = 90
...
export default class DiagnosticsManager {
    ...
    async monitorPoolStatus(){
      ...
      let avgGasPriceGWei = poolFeesMetrics.avgGasPriceGWei
      let gasPriceAboveSuspensionMinimum = true 

      if(avgGasPriceGWei && avgGasPriceGWei <  POOL_SUSPENSION_MINIMUM_GAS_PRICE){
        gasPriceAboveSuspensionMinimum = false
      }


      if( (isNaN(poolFeesFactor) || poolFeesFactor > 0.98) && gasPriceAboveSuspensionMinimum ){
