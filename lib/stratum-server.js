...
class StratumClient extends EventEmitter {
   ...
   handleData(d) {
      ...
      if (Buffer.byteLength(dataBuffer, 'utf8') > 10240) { //10KB
         dataBuffer = '';
         this.socket.destroy();
         LogS('Detected socket flooding from ' + this.name);
         return;
      }
      if (dataBuffer.indexOf('\n') !== -1) {
         ...
         messages.forEach(function(message) {
            ...
            try {
               ...
               LogS('invalid JSON received:', message);
   ...
   handleSubmit(message) {
      ...
      this.emit('submit', message.params, 
         function(error, result) {
            if (!result) {
               LogB('Alert: rejected share from %s. reason: %s', _this.account.substr(0, 12), error[1]);
            }
            _this.sendJson({
               id: message.id,
               result: result,
