...
module.exports =  class StratumServer {
   ...
   async handleNewConnection(socket) { 
      ...
      client.on('subscription', function(account) {
         _this.handleSubscription(client, account);
   ...
   async handleSubscription(client, account) {
      console.log('subscription account:', account)
      this.clients[account] = client;
      var minerData = await this.loadMinerDataFromRedis(account);
      ...
      client.target = this.getTargetFromDifficulty(varDiff);
