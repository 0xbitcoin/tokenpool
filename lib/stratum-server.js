...
module.exports =  class StratumServer {
   ...
   constructor (accountConfig, redisInterface) {
      ...
      logger.init(logFolder, 'stratum.log');
   }
   ...
   async start() {
      ...
      // handle incoming ipc data from other modules: for new challenge #, ETH block number, and updated vardiff for miners
      this.setupIpc();
   }
   ...
   setupIpc() {
      ...
      this.subscriber.on('message', function(channel, message) {
         ...
         switch (channel) {
            ...
            case 'challengeNumber':
               ...
               LogD('challenge number:', message.substr(0, 12));
               _this.currentChallenge = message;
               _this.broadcastWork();
               break;

            case 'ethBlockNumber':
               // LogD('ETH block number:', message);
               _this.ethBlockNumber = message;
               break;

            case 'varDiff':
               // updated vardiff for a specific miner. 
               // expecting data = {miner: ..., varDiff: ..., target: ...}
               try {
                  var workData = JSON.parse(message);
                  workData.miner = workData.miner.toLowerCase();
               } 
               catch(err) {
                  LogB('Error in Stratum-server::setupIPC - invalid JSON received:', message);
                  break;
               }
               LogD('updated vardiff for', workData.miner.substr(0, 12), ', varDiff:', workData.varDiff);
               // is the miner currently connected?
      ...
      this.subscriber.subscribe(['challengeNumber', 'ethBlockNumber', 'varDiff']);
   ...
   async handleShareSubmit(client, params, resultCallback) {
      ...
      var nonce            = params[0];
      var minerEthAddress  = params[1].toLowerCase();
      var digest           = params[2];
...
class StratumClient extends EventEmitter {
   ...
   handleSubscribe(message) {
      var _this = this;
      this.account = message.params[0].toLowerCase();
