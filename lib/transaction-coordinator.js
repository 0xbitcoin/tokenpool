  ...
  */
  ...
  async monitorMinedPayments()
  {
    ...
    var self = this ;
     ...
     setTimeout(function(){self.monitorMinedPayments()},5*60*1000)  //five minutes
     ...
     async checkBatchPaymentsStatus()
     {
       ...
       for( var element of unconfirmedBatches )   {
         ...
         console.log(self.getPaymentContractAddress(), element.id)
          ...
          await self.mongoInterface.upsertOne('payment_batch',{id: batchData.id},  element  )
       ...
       var self = this ;
        ...
        //IMPLEMENT ^
         ...
         try {
          ...
          for(element in unconfirmedBatches)
          {
            ...
            if(element.broadcastedAt == null || element.broadcastedAt < currentEthBlock - REBROADCAST_WAIT_BLOCKS){
                ...
                if(!complete )
                {
                  await self.broadcastPaymentBatch(element)
