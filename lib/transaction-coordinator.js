...
export default class TransactionCoordinator  {
  ...
  */
   ...
   static  async monitorMinedPayments(paymentContract, mongoInterface, poolConfig)
  {
      ...
      try {
           ...
           await TransactionCoordinator.batchMinedPayments( mongoInterface, poolConfig  )
  ...
  static async monitorMinedSolutions( mongoInterface, poolConfig )
  {
   ...
   //getAllUnconfirmedSubmittedSolutions
   let solution_txes = await TransactionHelper.findTransactionsWithQuery({ txType:'solution', status:'pending' } ,mongoInterface)
    ...
    {
      //look for receipt 
       var response = await TransactionHelper.checkPendingSolutions( solution_txes , poolConfig, mongoInterface)
    }
   }catch(e)
   {
   console.log('error',e)
  ...
  */
     static async batchMinedPayments( mongoInterface, poolConfig )
     {
      ...
      if( unbatched_pmnts.length >= MIN_PAYMENTS_IN_BATCH)
      {
        ...
        await mongoInterface.upsertOne('payment_batch',{id: batchData.id},   batchData  )
...
//types are 'transfer','solution'
  ...
  async broadcastQueuedMintTransactions(mongoInterface, poolConfig){
   ...
   //var transactionStats = await this.getTransactionStatistics(mongoInterface);  

   // let queuedMintTransactions = await TransactionHelper.findMintSolutionsWithQuery({status:'queued'},mongoInterface)
    ...
    var hasQueuedTransaction = (nextQueuedTransactionData != null)
       ...
       if( hasQueuedTransaction && !hasPendingTransaction ){
          ...
          try{
             ...
             var successful_broadcast = await this.broadcastMintTransaction(nextQueuedTransactionData, mongoInterface, poolConfig);
  ...
  /* async broadcastQueuedPaymentTransactions(){
   ...
   async broadcastMintTransaction(transactionPacketData, mongoInterface, poolConfig)
   {
     ...
     // }
        ...
        await TransactionHelper.updateOneTransactionById(transactionPacketData._id, {status:'pending', txHash: tx_hash}, mongoInterface)
          ...
          */
            ...
            //resave
        //var storage_success = await TransactionHelper.storeEthereumTransaction(tx_hash,transactionPacketData);
      ...
      if(packetData.receiptData.mined == false && packetData.receiptData.lost != true  ){
             ...
             if(receipt!=null)
             {
               console.log('got receipt storing packet ')
                ...
                await TransactionHelper.storeEthereumTransaction(txHash,packetData);
