...
export default class TransactionCoordinator  {
  ...
  async update()
  {
            ...
            setInterval(function(){this.broadcastQueuedMintTransactions(this.mongoInterface)}.bind(this),5000)
...
//types are 'transfer','solution'
  ...
  async broadcastQueuedMintTransactions(mongoInterface){
    ...
    var nextQueuedTransactionData = await TransactionHelper.findNextQueuedSolution(mongoInterface) //JSON.parse(nextQueuedTransactionDataJSON)
    ...
    var hasQueuedTransaction = (transactionStats.queuedCount > 0)
       ...
       if( hasQueuedTransaction && !hasPendingTransaction ){
          ...
          try{
             ...
             var successful_broadcast = await this.broadcastMintTransaction(nextQueuedTransactionData);
  ...
  /* async broadcastQueuedPaymentTransactions(){
   ...
   async broadcastMintTransaction(transactionPacketData)
   {
