...
export default class TransactionCoordinator  {
  ...
  constructor(mintingWeb3, paymentsWeb3, poolConfig,   mongoInterface )
  {


    this.mintingWeb3 = mintingWeb3;
    this.paymentsWeb3 = paymentsWeb3
    ...
    this.paymentContract = ContractHelper.getBatchedPaymentsContract(this.paymentsWeb3,this.poolConfig )
      ...
      if(!pendingTransaction.block || pendingTransaction.block < currentEthBlock-500){
        ...
        if(paymentWithUUIDWasSuccessful){
          ...
          await mongoInterface.updateOne('transactions',{_id:pendingTransaction._id}, 
            {block: currentEthBlock, status:'success'} )
...
*/
   ...
   //fix me up ! 
     ...
     {
       try{
          var data = await this.mintingWeb3.eth.getTransaction(tx_hash);
        }catch(err)
        {
   ...
   async requestTransactionReceipt(tx_hash)
   {
      ...
      try{
      var receipt = await this.mintingWeb3.eth.getTransactionReceipt(tx_hash);
      }catch(err)
      {
