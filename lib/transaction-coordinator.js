  ...
  async broadcastQueuedTransactions(){
   ...
   async broadcastTransaction(transactionPacketData)
   {
    ...
    if(tx_hash == null){
      ...
      this.storeNewSubmittedSolutionTransactionHash(tx_hash, txData.tokenReward, txData.challenge_number )
   ...
   async updateBroadcastedTransactionStatus() // this must be broken !!
   {
     ...
     //implement me
      ...
      /*  var ethereumTransactions = await ethereumTransactionHashes.map(async function(item){
          return  await this.redisInterface.findHashInRedis

        })

        var ethereumTransactions: number[] = await Promise.all(arr.map(async (item): Promise<number> => {
            return await this.redisInterface.findHashInRedis(item);
      ...
      // console.log('queuedTransactions',queuedTransactions)
       //console.log('ethereumTransactions',ethereumTransactions)
...
//        console.log('transactionPacketsData',transactionPacketsData)
   ...
   //for vue
      async getQueuedTransactionRenderList()
      {

      },

      async getActiveTransactionRenderList()
      {
