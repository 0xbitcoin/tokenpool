...
//types are 'transfer','solution'
  async addTransactionToQueue(txType,txData)
  {
    ...
    var packetData = {block: blockNum, txType: txType, txData: txData, receiptData: receiptData}
  ...
  async storeEthereumTransaction(tx_hash,packetData)
  {
    ...
    await this.redisInterface.pushToRedisList('active_transactions_list', JSON.stringify(packetData))
     ...
     for(i in ethereumTransactionHashes)
     {
      ...
      if(packetData.receiptData.mined == false){
             ...
             if(receipt!=null)
             {
               ...
               if(lastRebroadcastBlock == 0)
               {
                 lastRebroadcastBlock = current_block -110 ;
               }


               //rebroadcast
               if( current_block - pending_block > 100  && pending_block > 0  && current_block - lastRebroadcastBlock > 100 )
               {
