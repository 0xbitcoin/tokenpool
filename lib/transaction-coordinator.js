...
/*
  ...
  async broadcastQueuedMintTransactions(mongoInterface, poolConfig){
   ...
   async broadcastQueuedBatchedPaymentTransactions(mongoInterface, poolConfig){
    ...
    let pendingTransactions = await TransactionHelper.findTransactionsWithQuery({txType:'batched_payments',status:'pending'},mongoInterface)

    var hasPendingTransaction = (pendingTransactions.length > 0);

  

    var nextQueuedTransactionData = await TransactionHelper.findOneTransactionWithQuery({txType:'batched_payments',status:'queued'},mongoInterface) //JSON.parse(nextQueuedTransactionDataJSON)
